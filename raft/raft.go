...
func (r *Raft) applyCommand(req *pb.ApplyCommandRequest) (*pb.ApplyCommandResponse, error) {
	...
	newLog := &pb.Entry{Id: lastId + 1, Term: r.currentTerm, Data: req.GetData()}
...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	// if a candidate or leader is out of date, turn to follower state
	if r.currentTerm < req.GetTerm() {
		// toFollower: change to Follower & set curretTerm = term & voteFor = 0
	...
	// Candidate wait for vote but new Leader came out and sent heartbeat (has the same term)
	if r.currentTerm == req.GetTerm() && r.state != Follower {
	...
	if r.commitIndex < req.GetLeaderCommitId() {
		lastLogId, _ := r.getLastLog()
		r.setCommitIndex(min(req.GetLeaderCommitId(), lastLogId))
		// applies logs between (lastApplied, commitIndex] (applyCh stored commit data)
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	if lastLogTerm > req.GetLastLogTerm() || (lastLogTerm == req.GetLastLogTerm() && lastLogId > req.GetLastLogId()) {
		...
		// Up-to-date: later term, then longer log
...
func (r *Raft) broadcastRequestVote(ctx context.Context, voteCh chan *voteResult) {
	...
	lastLogId, lastLogTerm := r.raftState.getLastLog()
...
func (r *Raft) handleVoteResult(vote *voteResult, grantedVotes *int, votesNeeded int) {
	...
	if r.currentTerm < vote.GetTerm() {
		r.raftState.toFollower(vote.GetTerm())
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		// get entries from the start id to the end
		entries := r.getLogs(r.nextIndex[peerId])
		prevEntry := r.getLog(r.nextIndex[peerId] - 1)
		req := &pb.AppendEntriesRequest{
			Term:           r.currentTerm,
			LeaderId:       r.id,
			LeaderCommitId: r.commitIndex,
			PrevLogId:      prevEntry.GetId(),
...
func (r *Raft) handleAppendEntriesResult(result *appendEntriesResult) {
	...
	if r.currentTerm < result.GetTerm() {
	...
	if !result.GetSuccess() {
		...
		// what if reject by req.term > r.currentTerm ?
		...
		nextIndex := r.nextIndex[result.peerId] + uint64(len(entries))
	...
	for i := len(logs) - 1; i >= 0; i-- {
		...
		if replicas >= replicasNeeded {
			...
			// applyLogs should run in background
			go r.applyLogs(r.applyCh)
			return
		}
	}
